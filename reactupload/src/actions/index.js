...
export const signupAdmin = ({ password, email, adminKey }) => {
  console.log("signupAdmin called");
  return async (dispatch, getState) => {
    const url = "/signupadmin";
    console.log("url:", url);
    const enc_password = encryptPassword(password);
    const formData = new FormData();
    formData.append("email", email);
...
export const loginAdmin = ({ password, email, adminKey }) => {
  return async (dispatch, getState) => {
...
export const enterComment = ({ comment, userID, oid }) => {
  return async (dispatch, getState) => {
    const url = "/comment/" + oid;
    console.log("url:", url);
    const formData = new FormData();
...
export const getAllComment = (oid) => {
  return async (dispatch, getState) => {
    const url = "/comment/" + oid;
    console.log("url:", url);
    const response = await uploadEndpoint.get(url);
    ...
    dispatch({
      ...
      // payload: JSON.parse(response.data),
      payload:JSON.parse(response.data.comments),
    });
  };
};
